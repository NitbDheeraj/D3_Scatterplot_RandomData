...
var searchBox = d3.select("#search");
...
d3.csv("assets/data/data.csv").then(function(healthData, err){
	...
	// Create a color scale for the cluster labels
	var color = d3.scaleOrdinal()
				.domain(d3.range(d3.max(healthData, d => d.Clusters) + 1))
...
searchBox.on("blur", function() {
 // Get the search term entered by the user
  ...
  // Filter the data to find matching data points
  var filteredData = healthData.filter(function(d) {
    return d.Name.toLowerCase().includes(searchTerm);
  });
  
  // Highlight the matching data points
  circlesGroup.classed("highlighted", function(d) {
